function [LLik,Lm,Lf] = loglik_spec3(fxdpar,dat,Z0)
...
% Reshape vector into matrix  
    ...
    PE = zeros(4,1);
    PE(1) = Z0(fxdpar.N^2+fxdpar.N*2+fxdpar.k);
    PE(2) = Z0(fxdpar.N^2+fxdpar.N*2+fxdpar.k+1);
    PE(3) = Z0(fxdpar.N^2+fxdpar.N*2+fxdpar.k+2);
    PE(4) = 0;
    ...
    for r = 1:fxdpar.k
		...
		maxit = 1000;					% Set limit on nr iterations
 		...
 		while d>0.00000001,
			...
			Mm(:,:,r) = (s_m(:,:,r).^((1-PE(3))./(2-PE(1)-PE(2)))).*(s_f(:,:,r)'.^((1-PE(4))./(2-PE(1)-PE(2)))).*((h_f'./h_m).^((1-PE(2))./(2-PE(1)-PE(2)))).*(exp(Z+FE(r)).^(1/(2-PE(1)-PE(2))));
			Mf(:,:,r) = (s_m(:,:,r).^((1-PE(3))./(2-PE(1)-PE(2)))).*(s_f(:,:,r)'.^((1-PE(4))./(2-PE(1)-PE(2)))).*((h_m./h_f').^((1-PE(1))./(2-PE(1)-PE(2)))).*(exp(Z+FE(r)).^(1/(2-PE(1)-PE(2))));
